...
const VerifyEmailPage = () => {
    ...
    const status = useMemo<"loading" | "success" | "error" | "pending">(() => {
        if (!token) return "pending";
        if (verifyQuery.isPending) return "loading";
        if (verifyQuery.isError) return "error";
        return "success";
    }, [token, verifyQuery.isPending, verifyQuery.isError]);

    const message = useMemo(() => {
        if (!token) return "Check your email for verification link.";
        if (verifyQuery.isPending) return "Verifying your email...";
        if (verifyQuery.isError) {
            const error = verifyQuery.error as any;
            return error?.response?.data?.message || "Verification failed.";
        }
        return verifyQuery.data || "Email verified successfully!";
    }, [token, verifyQuery.isPending, verifyQuery.isError, verifyQuery.error, verifyQuery.data]);
            ...
            <div className="w-full max-w-md bg-card border border-border p-8 text-center space-y-6">
                ...
                {status === "pending" && (
                    <div className="flex flex-col items-center space-y-4">
                        ...
                        <Button asChild variant="outline" className="w-full rounded-none mt-4 cursor-pointer">
                ...
                {status === "success" && (
                    <div className="flex flex-col items-center space-y-4">
                        ...
                        <p className="text-muted-foreground">{message}</p>
                        <Button asChild className="w-full rounded-none mt-4 cursor-pointer">
